...
public class AStarSolver {
    ...
    public final static AStarSolver SOLVED=new AStarSolver();


    public AStarSolver(AStarSolver toClone) {
    ...
    public boolean equals(Object o) {
        if(o instanceof AStarSolver) {
            for(TilePos p: allTilePos()) {
                if( this.tile(p) != ((AStarSolver) o).tile(p)) {
                    return false;
                }
            }
            return true;
     ...
     * @return
     */
    public AStarSolver moveClone(TilePos p) {
        AStarSolver out = new AStarSolver(this);
        out.move(p);
        return out;
    }
    ...
    public List<AStarSolver> allAdjacentPuzzles() {
        ArrayList<AStarSolver> out = new ArrayList<AStarSolver>();
     ...
     * @return
     */
    public List<AStarSolver> dijkstraSolve() {
        Queue<AStarSolver> toVisit = new LinkedList<AStarSolver>();
        HashMap<AStarSolver, AStarSolver> predecessor = new HashMap<AStarSolver, AStarSolver>();
        toVisit.add(this);
        predecessor.put(this, null);
        int cnt=0;
        while( toVisit.size() > 0) {
            AStarSolver candidate = toVisit.remove();
            cnt++;
            if( cnt % 10000 == 0) {
            ...
            if( candidate.isSolved() ) {
                ...
                LinkedList<AStarSolver> solution = new LinkedList<AStarSolver>();
                AStarSolver backtrace=candidate;
            ...
            for(AStarSolver fp: candidate.allAdjacentPuzzles()) {
     ...
     * @return
     */
    public List<AStarSolver> aStarSolve() {
        HashMap<AStarSolver, AStarSolver> predecessor = new HashMap<AStarSolver, AStarSolver>();
        HashMap<AStarSolver,Integer> depth = new HashMap<AStarSolver,Integer>();
        final HashMap<AStarSolver,Integer> score = new HashMap<AStarSolver,Integer>();
        Comparator<AStarSolver> comparator = new Comparator<AStarSolver>() {
            @Override
            public int compare(AStarSolver a, AStarSolver b) {
                return score.get(a) - score.get(b);
            }
        };
        PriorityQueue<AStarSolver> toVisit = new PriorityQueue<AStarSolver>(10000,comparator);
    ...
    private static void showSolution(List<AStarSolver> solution) {
        if (solution != null ) {
            System.out.printf("Success!  Solution with %d moves:\n", solution.size());
            for( AStarSolver sp: solution) {
                sp.show();
            }
        } else {
